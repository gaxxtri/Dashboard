# Sunlytics 2.0 - Streamlit Dashboard (Optimized for Cloud)
...
# ------------------- Background & fonts -------------------
def set_bg(png_url):
    # Load background image from local or URL
    try:
        import requests
        b64 = base64.b64encode(requests.get(png_url).content).decode()
    except Exception:
        b64 = ""
    css = f"""
    <style>
...
# Use public URL of your sunshine image or raw GitHub file
set_bg("https://raw.githubusercontent.com/gaxxtri/Dashboard/main/sunshine.jpeg")

# ------------------- Dataset URLs -------------------
URBAN_FILE_ID = "1AN23RdxSg_MvQ9BKRvqV7mJAeHwFjd0S"
VILLAGE_FILE_ID = "1pEIzN_vbarlj-tBUv8CYN1IR56r2gUUS"

urban_url = f"https://drive.google.com/uc?id={URBAN_FILE_ID}"
village_url = f"https://drive.google.com/uc?id={VILLAGE_FILE_ID}"

# ------------------- Load Data -------------------
@st.cache_data
def load_urban_data(url):
    df = pd.read_csv(url)
    months = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']
    numeric_guess = ['slab_max_kwh','rate_per_kwh','area_m2','centroid_lat','centroid_lon','rooftop_area_deg2','payback_years'] + [m+'_kwh' for m in months]
...
urban_df, months = load_urban_data(urban_url)
village_df = load_village_data(village_url)
...
def compute_urban_metrics(df):
    ...
    top_sum = top10['slab_max_kwh'].sum()
    other_sum = total_energy - top_sum
    return {"total_energy": total_energy, "total_savings": total_savings, "total_co2": total_co2,
            "avg_energy": avg_energy, "top10": top10, "top_sum": top_sum, "other_sum": other_sum,
            "n_buildings": len(df)}
...
def compute_village_metrics(df):
    ...
    return {"avg_urgency": avg_urgency, "avg_access": avg_access, "low_elec": low_elec,
            "urgency_series": urgency_series, "top10": top10, "n_villages": len(df)}
...
def create_building_type_pie(df):
    counts = df['building_type'].value_counts()
    fig = px.pie(values=counts.values, names=counts.index, title="Buildings by Type",
...
def create_village_map(df):
    ...
    for r in df.itertuples(index=False):
        ...
        urgency = getattr(r,'urgency_score',0)
        if pd.isna(lat) or pd.isna(lon): continue
        color = 'red' if urgency>=7 else ('orange' if urgency>=4 else 'green')
        folium.CircleMarker(location=[lat,lon], radius=6, color=color, fill=True, fill_opacity=0.7,
                            popup=f"{name} — Urgency: {urgency}").add_to(cluster)
    return m
...
if page=="Welcome":
    ...
    with col1:
        ...
        st.plotly_chart(create_building_type_pie(urban_df), use_container_width=True)
    with col2:
        ...
        st.plotly_chart(create_village_elec_pie(village_df), use_container_width=True)
